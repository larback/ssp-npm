...
public:
  ...
  bool getMelhorou(unsigned i) const;  // Returns if the last local search improve better results
...
private:
  ...
  std::vector< std::pair< bool, unsigned > > melhorou;
  ...
  void setMelhorou(unsigned i, bool b);
...
Population::Population(const Population& pop) :
		...
		fitness(pop.fitness), melhorou(pop.melhorou) {
}

Population::Population(const unsigned n, const unsigned p) :
		population(p, std::vector< double >(n, 0.0)), fitness(p), melhorou(p) {
...
bool Population::getMelhorou(unsigned i) const {
  return melhorou[i].first;
}
...
void Population::setMelhorou(unsigned i, bool b){
  melhorou[i].first = b;
  melhorou[i].second = i;
}
