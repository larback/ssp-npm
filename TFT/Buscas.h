...
void IBS(std::vector < std::vector <int> >& maquinas, std::vector < std::pair <double,int> >& idx_maquinas, double& makespan, std::vector<std::vector<unsigned>> tProcessamento){
    ...
    for (salto = 0; salto <= m-2; salto++){
      int melhorPosicao = 0;
      std::sort(idx_maquinas.rbegin(), idx_maquinas.rend());
      critica = idx_maquinas.begin() + salto;
      ...
      c4 = completionTime(tProcessamento,mCritica,critica->second-1) + completionTime(tProcessamento,mBest,best->second-1);
      ...
      // cout << idxTask << " " << mCritica.size() << endl;
...
void EFB(std::vector < std::vector <int> >& maquinas, std::vector < std::pair <double,int> >& idx_maquinas, double& makespan, std::vector<std::vector<unsigned>> tProcessamento){
    ...
    while(melhorou){
        ...
        for (int i=0; i < mCritica.size(); ++i){
          ...
          // Confere elegibilidade da máquina best para a tarefa selecionada
          if(ferramentasJob[tCritica]>c[best->second-1])
            continue;
          ...
          for (int j=0; j < mBest.size(); ++j){
            int tBest = mBest[j];
            // Confere elegibilidade da máquina critica para a tarefa selecionada
            if(ferramentasJob[tBest]>c[critica->second-1])
              continue;
            ...
            if ((b1) && (b2) ){
              ...
              mBest[j]=tCritica;
              mCritica[i]=tBest;
              double c3 = completionTime(tProcessamento, mCritica,critica->second-1);
              double c4 = completionTime(tProcessamento, mBest, best->second-1);
              if ((c3+c4)<(c1+c2)){
